...
@given(u'o veículo com identificação "{id}", placa "{placa}" e quilometragem "{km}"')
def step_impl(context, id, placa, km):
...
@then(u'o veículo cadastrado deve ter os dados identificação "{id}", placa "{placa}" e quilometragem "{km}"')
def step_impl(context, id, placa, km):
    resultado = context.cadastro.consultar(id)
    assert (resultado.placa == placa) and (resultado.km == km)
       
@given(u'os cadastros dos dados de veículos')
def step_impl(context):
    context.cadastro = CadastroVeiculo()
    for dado in context.table:
        veiculo = Veiculo(dado['id'], dado['placa'], dado['km'])
        context.resultado = context.cadastro.inserir(veiculo)

@when(u'o veículo com id "{id}" for removido')
def step_impl(context, id):
    context.cadastro.remover_por_id(id)
    
@then(u'a quantidade de veículos cadastrados deve ser {quantidade}')
def step_impl(context, quantidade):
    resultado = context.cadastro.listar_todos()
    assert len(resultado) == int(quantidade)
    
@when(u'listados todos os veículos')
def step_impl(context):
